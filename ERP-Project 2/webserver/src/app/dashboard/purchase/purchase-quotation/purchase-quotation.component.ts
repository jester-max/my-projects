...
export class PurchaseQuotationComponent {
  ...
  toggle(){
    console.log('aaa')
    if (this.create){
      this.service.getPurchaseQuatation(this.router.snapshot.params['_id']).subscribe((res:any)=>{
        console.log(res)
        if (res.Status === "SUCCESS"){
  ...
  submit(data:any){
    this.service.postPurchaseQuatation(ValidationFunction(this.data),this.router.snapshot.params['_id']).subscribe((res:any)=>{
      if (res.Status === 'SUCCESS'){
        this.toast.success({detail:'Success',summary:res.Status})
      }else {
        console.log(res)
