...
@Component({
  selector: 'app-sale-transport',
  templateUrl: './sale-transport.component.html',
  styleUrls: ['./sale-transport.component.css']
})
export class SaleTransportComponent {
  data:any
  data2:any
  ...
  toggle(){
    console.log('aaa')
    if (this.create){
      this.service.getSaleTransport(this.router.snapshot.params['_id']).subscribe((res:any)=>{
        console.log(res)
        if (res.Status === "SUCCESS"){
  ...
  submit(data:any){
    this.service.postSaleTransport(ValidationFunction(this.data),this.router.snapshot.params['_id']).subscribe((res:any)=>{
      res.DATA
      if (res.Status === 'SUCCESS'){
        // this.toast.success({detail:'Success',summary:res.Status})
      }else {
