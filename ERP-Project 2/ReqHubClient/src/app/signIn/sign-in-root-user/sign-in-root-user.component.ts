...
export class SignInRootUserComponent {
  ...
  SignInRootUser(SignInRootUserInfo:any) {
    console.log(SignInRootUserInfo)
    this.Auth.SignInRootUser(SignInRootUserInfo).subscribe((Response:any)=>{
      this.Response = Response
      console.log(this.Response)
      if (this.Response.status === 'SUCCESS'){
        if (this.Response.data.completeConfiguration === 'pending'){
          window.location.href = `http://localhost/mainModules/${this.Response.data.accessToken}`;
        } else if (this.Response.data.completeConfiguration === 'complete'){
          this.router.navigate(['root',this.Response.data.accessToken])
          // window.location.href = `http://localhost/landing-page/${this.Response.data.accessToken}`;
        } else {
          this.router.navigate(['select-service',this.Response.data.accessToken])
        }
        this.cookies.set('accessToken',this.Response.data.accessToken)
      }
