...
exports.createQuotationData = async function (req,res,next) {
    try {
        ...
        if(tenant){
            ...
            const result = await createTable('sale_quotation','sale_quotation', modifiedData, null,null,modifiedData,creatingTableArray);
            ...
            for (let i = result.length-1; i >= 0 ; i--) {
                ...
                if(i===result.length-1){
                    ...
                    const substringToRemove = "sale_quotation_";
...
exports.getQuotationData = async function (req,res) {
    try {
        ...
        const sqpQuery = `select * from sale_quotation`;
        ...
        if (tenant) {
            const quotationModel = await getInvoiceModelPostgres(tenantID);

            const getArray = await quotationModel.query(sqpQuery);
