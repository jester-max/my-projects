...
const { getTenantModelMongodb,getTransportModelMongodb } = require('../../databases/tenant.mongodb.connection');
...
exports.createTransportData = async function (req,res,next) {
    try {
        ...
        if(tenant){
            ...
            const transportModel = await getTransportModelMongodb(tenantID);

            // Fetch default values from MongoDB collection
            const mongoSchema = await transportModel.findOne({}).sort({_id:-1})
            ...
            const result = await createTable('sale_transport','sale_transport', modifiedData, null,null,modifiedData,creatingTableArray);
            ...
            for (let i = result.length-1; i >= 0 ; i--) {
                ...
                if(i===result.length-1){
                    ...
                    const substringToRemove = "sale_transport_";
...
exports.getTransportData = async function (req,res) {
    try {

        const sqpQuery = `select * from sale_transport`;

        const tenantID = req.params.tenantID;

        const tenatModel = await getTenantModelMongodb(tenantID);

        const tenant = await tenatModel.findOne({ tenantID: tenantID });

        if (tenant) {
            const transModel = await getInvoiceModelPostgres(tenantID);

            const getArray = await transModel.query(sqpQuery);
