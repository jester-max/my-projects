...
const { getTenantModelMongodb,getEmployeeModelMongodb } = require('../../databases/tenant.mongodb.connection');
...
exports.createEmployeeData = async function (req,res,next) {
    try {
        ...
        if(tenant){
            ...
            const employeeModel = await getEmployeeModelMongodb(tenantID);

            // Fetch default values from MongoDB collection
            const mongoSchema = await employeeModel.findOne({}).sort({_id:-1})
            ...
            const result = await createTable('employee','employee', modifiedData, null,null,modifiedData,creatingTableArray);
            ...
            for (let i = result.length-1; i >= 0 ; i--) {
                ...
                if(i===result.length-1){
                    ...
                    const substringToRemove = "employee_";
...
exports.getEmployeeData = async function (req,res) {
    try {
        ...
        const sqpQuery = `select * from employee;`;
