...
const validateField = function (actaulFieldName, fieldConfig,primaryKeyEncountered = false) {

    if (!fieldConfig.type) {
        return createError(actaulFieldName, 'all fiedls empty', `'type' property is required`);
    }
    if (fieldConfig.primarykey) {
        if (primaryKeyEncountered) {
            return createError(actualFieldName, `'primarykey' should be specified only once`);
        }

        // Add specific terms and conditions for the primary key
        if (fieldConfig.unique !== true) {
            return createError(actualFieldName, `'primarykey' must be used with 'unique' set to true`);
        }

        if (fieldConfig.required !== true) {
            return createError(actualFieldName, `'primarykey' must be required`);
        }
    ...
    if (fieldConfig.type === 'array') {
        ...
        // Validate object type
        if (!fieldConfig.properties || typeof fieldConfig.properties !== 'object') {
            return createError(actaulFieldName, 'properties', `'properties' property is required for object type`);
        }
        ...
        for (const fieldName in fieldConfig) {
            if (fieldConfig.hasOwnProperty(fieldName)) {
                ...
                if ((hasLowercase && !hasUppercase) || (hasUppercase && !hasLowercase && !isStringType)) {
                    createError(actaulFieldName, `'lowercase' is applicable only for string type and uppercase is false`);
                }

                if ((hasUppercase && !hasLowercase) || (hasLowercase && !hasUppercase && !isStringType)) {
                    createError(actaulFieldName,  `'uppercase' is applicable only for string type and lowercase is false`);
                }
                ...
                if (fieldConfig.sign && !fieldType.includes('number')) {
                    createError(actaulFieldName, `'sign' is applicable only for number type`);
                }

                // Check boolean conditions
                if (fieldConfig.sign && fieldConfig.unsign && fieldType.includes('number')) {
                    createError(actaulFieldName, `'select only one sign or unsign for number type`);
                }

                if (fieldConfig.unsign && !fieldType.includes('number')) {
                    createError(actaulFieldName, `'unsign' is applicable only for number type`);
                }

                // Check float and integer conditions
                if (fieldConfig.float && !fieldType.includes('number')) {
                    createError(actaulFieldName, `'float' is applicable only for number type`);
                }

                if (fieldConfig.int && !fieldType.includes('number')) {
                    createError(actaulFieldName, `'int' is applicable only for number type`);
                }

                if (fieldConfig.celling && fieldConfig.float) {
                    createError(actaulFieldName, `'ceiling' is applicable only for integer type`);
                }

                if (fieldConfig.celling && fieldConfig.int && fieldConfig.floor) {
                    createError(actaulFieldName, `'select only one celling or floor for integer type`);
                }

                if (fieldConfig.floor && fieldConfig.float) {
                    createError(actaulFieldName, `'floor' is applicable only for integer type not for float`);
                }

                if ((fieldConfig.steps === undefined || fieldConfig.steps === 0) && fieldConfig.float) {
                    createError(actaulFieldName, `'steps' is applicable only for float type with float and greater than 0`);
                }
                ...
                if (hasEnum && hasDefault && !hasEnumValues.includes(fieldConfig.default)) {
                    createError(actaulFieldName, `'enumValues' should be match default value`);
                }

                if (hasEnum && (!hasEnumValues || !Array.isArray(hasEnumValues))) {
                    createError(actaulFieldName, `'enumValues' should be an array when 'enum' is true`);
                }
                ...
                if (hasPrimaryKey && !hasIndexes) {
                    createError(actaulFieldName, `'primarykey' and 'indexes' used together`);
                }
                ...
                if (hasForeignKey && hasUnique && !isRequired) {
                    createError(actaulFieldName, `'fkey' requires requires and not to be set to true with unique`);
                }

                // Check unique property
                if (hasUnique && !hasIndexes) {
                    createError(actaulFieldName, `'unique' requires 'indexes' to be set to true`);
                }

                // Check for auto-increment conflicts
                if (fieldConfig.autoIncrement && isStringType) {
                    createError(actaulFieldName, `'autoIncrement' not be set to true with type string`);
                }
                ...
                if ((hasMin || hasMax) && !fieldType.includes('number')) {
                    createError(actaulFieldName, `'min' and 'max' are applicable only for number type`);
                }

                if (hasMin && hasMax && fieldConfig.min >= 0 && fieldConfig.max < fieldConfig.min && fieldType.includes('number')) {
                    createError(actaulFieldName, `'min' and 'max' are not same and always max is greater`);
                }
                ...
                // Check for minlength and maxlength conflicts
                const hasMinLength = fieldConfig.minlength !== undefined;
                const hasMaxLength = fieldConfig.maxlength !== undefined;

                if ((hasMinLength || hasMaxLength) && !['string', 'number', 'email', 'date'].some(type => fieldType.includes(type))) {
                    createError(actaulFieldName, `'minlength' and 'maxlength' are applicable only for string or number type`);
                }
...
module.exports = {validateSchema,validateField,validateActualData,removeSpacesInKeys}
