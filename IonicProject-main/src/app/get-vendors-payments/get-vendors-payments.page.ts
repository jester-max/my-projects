...
@Component({
  selector: 'app-get-vendors-payments',
  templateUrl: './get-vendors-payments.page.html',
  styleUrls: ['./get-vendors-payments.page.scss'],
  providers: [DatePipe]
})
export class GetVendorsPaymentsPage implements OnInit {
  results:any
  showList: any = false;
  ...
  getVendorAccount(){
    this.service.findVendorReport(this.perPage,this.pageSize).subscribe((res:any)=>{
      console.log(res)
      this.message=res.message
      this.toastOpen(res.status)
      this.GetVendorAccountData = res.data.data
    })
  }
  ...
  paginateItem(event:any){
    setTimeout(() => {
      ...
      if (this.searchText===''){
        this.service.findVendorReport(this.perPage,this.pageSize).subscribe((res:any)=>{
          this.GetVendorAccountData = this.GetVendorAccountData.concat(res.data.data)
          console.log(res)
          event.target.complete();
        })
      }else{
        this.service.searchVendorAccount(this.searchText, this.perPage, this.pageSize).subscribe((res: any) => {
          this.GetVendorAccountData = this.GetVendorAccountData.concat(res.data.data)
          console.log(res)
          event.target.complete();
        })
  ...
  dateFilter() {
    ...
    this.service.DateFilterItem(this.fromDate,this.toDate).subscribe((res:any)=>{
      this.GetVendorAccountData = res.data
      if (this.GetVendorAccountData.length===0){
        this.message = 'DATA IS NOT AVAILABLE!'
        this.toastOpen('PENDING')
      }
      this.DateFilter = false
      console.log(this.GetVendorAccountData)
    })
  }
}
