...
@Component({
  selector: 'app-get-raw-materials',
  templateUrl: './get-raw-materials.page.html',
  styleUrls: ['./get-raw-materials.page.scss'],
  providers: [DatePipe]
})
export class GetRawMaterialsPage implements OnInit {
  ...
  getItem(){
    this.service.GetMaterialItem(this.perPage,this.pageSize).subscribe((res:any)=>{
      console.log(res)
  ...
  paginateItem(event:any){
    setTimeout(() => {
      ...
      if (this.searchText===''){
        ...
        this.service.SearchMaterialItem(this.searchText, this.perPage, this.pageSize).subscribe((res: any) => {
          this.GetItemData = this.GetItemData.concat(res.data.data)
          console.log(res)
          event.target.complete();
        })
