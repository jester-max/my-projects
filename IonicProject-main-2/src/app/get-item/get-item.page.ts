...
@Component({
  ...
  templateUrl: './get-item.page.html',
  styleUrls: ['./get-item.page.scss'],
  providers: [DatePipe]
})
export class GetItemPage implements OnInit {
  ...
  getItem(){
    this.service.GetItem(this.perPage,this.pageSize).subscribe((res:any)=>{
      console.log(res)
  ...
  faultyItem(item:any){
    this.service.FaultyItem(item).subscribe((res:any)=>{
      console.log(res)
    })
  }

  statusItem(){
    this.service.StatusItem().subscribe((res:any)=>{
      console.log(res)
  ...
  stop(reason:any,productId:any,status:any){
    var rejection = { varifyProductStatus: status, reason: reason, productId:productId }
    this.service.RejectItem(rejection).subscribe((res:any)=>{
